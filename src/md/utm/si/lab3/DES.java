...
public class DES {
    ...
    private static void initGUI() {
        final GUI inputForm = new GUI();
        inputForm.frm.setVisible(true);

        // butonul de criptare
        inputForm.Encrypt.addActionListener(e -> {
            takeInput(inputForm);
            if(key!=null)
                encrypt(inputForm);
        });

        // butonul de decriptare
        inputForm.Decrypt.addActionListener(e -> {
            takeInput(inputForm);
            if(key!=null)
                decrypt(inputForm);
        });
    ...
    private static void takeInput(GUI inputForm) {
        //verificare dacă datele introduse sunt hexazecimale
        boolean isInputHex = inputForm.tInput.getText().matches("[0-9A-F]+");
        boolean isKeyHex = inputForm.tKey.getText().matches("[0-9A-F]+");

        //verificare dacă lungimea este egală cu 16 caractere (64 de biți)
    ...
    private static void setKeys(GUI inputForm) {
        int i;
    ...
    private static void setData(GUI inputForm, Boolean encrypt) {
        int i=1;
        String templ,tempr;
        IPerm = "";
    ...
    private static void encrypt(GUI inputForm) {
    ...
    private static void decrypt(GUI inputForm) {
        inputForm.tOutput.setText("");
